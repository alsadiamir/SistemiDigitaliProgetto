...
)
{
	...
	ap_int<MUL_BITS> out_pixel=0;

	//senza separazione

	printf("Elementi matriciali per prodotto: \n");

	Loop_convolution_rows: for(int j=0; j < KERNEL_HEIGHT; j++)
		Loop_convolution_cols:	for(int i = 0; i < KERNEL_WIDTH; i++)
									out_pixel += window[j][i] * kernel[j][i];
...
#pragma HLS INTERFACE axis port=in_img
	...
	static pixel line_buffer_temp [KERNEL_HEIGHT - 1];
	...
	for(int i=0;i<KERNEL_HEIGHT;i++)
		...
		#pragma HLS PIPELINE II=1
			float conv = kernel[i][j]/kernel_sum;
			kernel[i][j]=compute_signed(conv);
		}
...
#pragma HLS PIPELINE II=1
	...
	if(col < IMG_WIDTH)
		for(int ii = 0; ii < KERNEL_HEIGHT - 1; ii++)
			window[ii][KERNEL_WIDTH - 1] = line_buffer_temp[ii];
	...
	{
		int in_temp = in_img[row*IMG_WIDTH+col];
	...
	if (row >= KERNEL_MID_HEIGHT && col >= KERNEL_MID_WIDTH){
		...
		pixel out = pixel_weighted_average(kernel, kernel_sum, kernel_off, window,rand);
